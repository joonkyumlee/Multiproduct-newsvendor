...
for z=Start_number:End_number
    ...
    p_i(1:n+1)=DATA(z,j:j+n); j=j+n+1; % choice probability under full assortment vector 
    ...
    Num_Customer=poissrnd(mu,[1,number_iteration]); % random number of customers; We actually preset this for the paper in Num_Customer.mat to allow uniform comparison across different methods and used load('Num_Customer.mat')
    Prob=Choice_Prob_Calculator_Nested_logit(n, p_i, Nest_info, N_nest, Lambda); % choice probability calculation
