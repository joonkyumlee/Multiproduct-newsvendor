...
Eps=0.00000000001; % Small number to avoid numerical problems
...
while Gamma_Candi>0
    ...
    % Algorithm 5 Step 2
    [Gamma_Candi, Opt_q_Candi]=max((Underage_minus.*(Underage_minus>Eps))./(Underage_minus.*(Underage_minus>Eps)-(Overage_minus.*(Overage_minus<-Eps))));
    if Gamma_Candi >0
        Opt_q(j)=Opt_q_Candi;
        Gamma(j)=Gamma_Candi;
    end;
